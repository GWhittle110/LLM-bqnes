...
def torchTrain(model,
    ...
    train_counter = []
    test_losses = []
    test_counter = [i * len(train_loader.training_dataset) for i in range(n_epochs + 1)]
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if batch_idx % log_interval == 0:
                print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                    epoch, batch_idx * len(data), len(train_loader.training_dataset),
                           100. * batch_idx / len(train_loader), loss.item()))
                train_losses.append(loss.item())
                train_counter.append(
                    (batch_idx * 64) + ((epoch - 1) * len(train_loader.training_dataset)))
    ...
    def val(previous_loss):
        ...
        test_loss /= len(test_loader.training_dataset)
        test_losses.append(test_loss)
        print('\nTest set: Avg. loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
            test_loss, correct, len(test_loader.training_dataset),
            100. * correct / len(test_loader.training_dataset)))
